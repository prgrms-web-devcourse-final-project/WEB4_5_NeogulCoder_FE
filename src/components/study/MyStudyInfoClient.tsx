...
import { fetchMyCommunityList } from '@/lib/api/community';
...
export default function MyStudyInfoClient({
  initialMyCommunityData,
  extensionData,
  // isExtended,
  // isLeader,
  myData,
}: {
  initialMyCommunityData: MyListType;
  extensionData: ExtensionType;
  // isExtended: boolean;
  // isLeader: boolean;
  myData: MyInfoInStudyType;
}) {
  // 임시 CSR용
  const isExtended = extensionData.extended;
  const isLeader = myData.role === 'LEADER';
  ...
  >(initialMyCommunityData.postInfos);

  const [page, setPage] = useState(0);
  const [totalPages, setTotalPages] = useState(
    initialMyCommunityData.totalPage
  );
  ...
  const checkMyParticipation = useCallback(() => {
    extensionData.members.forEach((member) => {
      if (member.userId === me?.id) setIsParticipated(member.participated);
    });
  }, [extensionData.members, me?.id]);
  ...
  return (
    <div className='w-full'>
      ...
      {isOpenOutModal && (
        <StudyOutCheckModal studyId={studyId} onClose={handleCloseOutModal} />
      )}
    </div>
  );
}
